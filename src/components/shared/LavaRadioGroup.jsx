...
export const LavaRadioGroup = ({
  ...
  <div className="space-y-2">
      ...
      {options.map((option) => (
        <label key={option.id} className="flex items-center space-x-2 cursor-pointer">
          <div className="relative">
            ...
            />
            <div className={`w-5 h-5 rounded-full border-2 ${error ? 'border-red-500' : 'border-white/20'} peer-checked:border-[#FF842C]`} />
            <div
    ...
    {error && (
      <p className="text-red-500 text-sm mt-1">{typeof error === 'string' ? error : 'This field is required'}</p>
    )}
  </div>
);
