...
export const LavaWhitelistWithCaps = ({
  ...
  const getFilteredPolicyIds = (searchText, currentUniqueId) => {
    ...
    const availablePolicies = walletPolicyIds.filter(policy =>
      !usedPolicyIds.has(policy.policyId)
    );
    ...
    return availablePolicies.filter(policy =>
      ...
      policy.policyId.toLowerCase().includes(search)
    );
  };
      ...
      <div className="space-y-4">
        {whitelist.map(asset => (
          <div key={asset.id || asset.uniqueId} className="space-y-2">
            <div
              className="relative"
              ref={el => dropdownRefs.current[asset.uniqueId] = el}
            >
              <LavaInput
              ...
              {showDropdown[asset.uniqueId] && walletPolicyIds.length > 0 && (() => {
                      ...
                      const highlightText = (text, search) => {
                        ...
                        return parts.map((part, i) =>
                          part.toLowerCase() === search.toLowerCase() ?
                            <span key={i} className="bg-yellow-500 text-black">{part}</span> : part
                        );
                      };
