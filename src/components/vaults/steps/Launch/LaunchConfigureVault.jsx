...
export const LaunchConfigureVault = ({ data, setCurrentStep }) => (
  <section>
    <div className="rounded-t-[10px] py-4 px-8 flex justify-between bg-white/5">
      <p className="font-bold text-2xl">Configure Vault</p>
      <button className="flex items-center gap-2 text-dark-100" type="button" onClick={() => setCurrentStep(1)}>
        <Edit size={24} />
        Edit
      </button>
    </div>
    <div className="grid grid-cols-2 gap-8 rounded-b-[10px] bg-input-bg pt-4 pb-8 px-16">
      <div className="space-y-10">
        <div>
          ...
          <p className="text-[20px]">{data.name || 'No name'}</p>
        </div>
        <div>
          <p className="uppercase font-semibold text-dark-100">Vault type</p>
          <p className="text-[20px]">{VAULT_TYPE_OPTIONS.find(option => option.name === data.type)?.label}</p>
        </div>
        <div>
          <p className="uppercase font-semibold text-dark-100">Vault privacy</p>
          <p className="text-[20px]">{VAULT_PRIVACY_OPTIONS.find(option => option.name === data.privacy)?.label}</p>
        </div>
        <div>
          <p className="uppercase font-semibold text-dark-100">FT Token Ticker</p>
          <p className="text-[20px]">{data.vaultTokenTicker || 'Not set'}</p>
        </div>
        <div>
          <p className="uppercase font-semibold text-dark-100">Vault brief</p>
          <p className="text-[20px]">{data.description || 'No description'}</p>
        </div>
        <div>
